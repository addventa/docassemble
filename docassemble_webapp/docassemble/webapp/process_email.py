...
def main():
    ...
    if user is None:
        ...
        if len(role_list) == 0:
            role_list = ['user']
        user_info = dict(email=user.email, roles=role_list, the_user_id=user.id, theid=user.id, firstname=user.first_name, lastname=user.last_name, bnpp_roles=user.bnpp_roles, nickname=user.nickname, country=user.country, subdivisionfirst=user.subdivisionfirst, subdivisionsecond=user.subdivisionsecond, subdivisionthird=user.subdivisionthird, organization=user.organization)
