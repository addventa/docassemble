...
if [ "${DAUPDATEONSTART:-true}" = "true" ] && [ "${DAALLOWUPDATES:-true}" == "true" ]; then
    ...
    su -c "source \"${DA_ACTIVATE}\" && pip install --no-index --no-deps --upgrade pip==20.3 && python -m docassemble.webapp.update \"${DA_CONFIG_FILE}\" initialize" www-data || exit 1
